...
if ( is_singular( get_post_type() ) ) {
	...
	if ( ! empty( $post->post_parent ) ) {
		...
		if ( ! empty( $prev_link ) ) {
			...
			$attachment_navigation .= '<div class="post-navigation-sub"><span>' . esc_html__( 'Previous:', 'groundwp' ) . '</span></div>';
		...
		if ( ! empty( $next_link ) ) {
			...
			$attachment_navigation .= '<div class="post-navigation-sub"><span>' . esc_html__( 'Next:', 'groundwp' ) . '</span></div>';
			$attachment_navigation .= $next_link;
			$attachment_navigation .= '</div>';
		}

		if ( ! empty( $attachment_navigation ) ) {
			echo _navigation_markup( $attachment_navigation, $class = 'post-navigation', __( 'Post navigation', 'groundwp' ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		}
	}
