<?php
/**
 * GroundWP\GroundWP\Styles\Component class
 *
 * @package groundwp
 */
...
class Component implements Component_Interface, Templating_Component_Interface {
	...
	/**
	 * Inline main style file
	 */
	public function inline_styles() {
		global $wp_filesystem;
		if ( filter_var( get_theme_mod( $this->schema( 'settings-inline_css-slug' ), $this->schema( 'settings-inline_css-default' ) ), FILTER_VALIDATE_BOOLEAN ) ) {
			$main_style = get_theme_file_path( '/assets/css/global.min.css' );

			ob_start();
			include $main_style;
			$read_styles = ob_get_clean();

			// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			echo '<style type="text/css" media="screen">' . $read_styles . '</style>';
		}
	}

	/**
	 * WordPress customize_register hook callback function
	 *
	 * @param WP_Customize_Manager $wp_customize WP_Customize_Manager instance.
	 */
	public function customize_register( $wp_customize ) {
		$wp_customize->add_setting(
			$this->schema( 'settings-inline_css-slug' ),
			[
				'type'              => 'theme_mod',
				'default'           => $this->schema( 'settings-inline_css-default' ),
		...
		$wp_customize->add_section(
			...
			[
				'title' => esc_html__( 'CSS options', 'groundwp' ),
		...
		$wp_customize->add_control(
			$this->schema( 'settings-inline_css-slug' ),
			[
				'label'       => esc_html__( 'Enable inline CSS', 'groundwp' ),
				'description' => esc_html__( 'Inline main css file for the theme. This option will boost up first visit speed of your site, but will opt your styles out of caching.', 'groundwp' ),
				'type'        => 'checkbox',
				'default'     => $this->schema( 'settings-inline_css-default' ),
	...
	public function action_enqueue_styles() {
		...
		if ( ! empty( $google_fonts_url ) ) {
			wp_enqueue_style( 'groundwp-fonts', $google_fonts_url, [], null ); // phpcs:ignore WordPress.WP.EnqueuedResourceParameters.MissingVersion
		}
		...
		foreach ( $css_files as $handle => $data ) {
			...
			$version = groundwp()->get_asset_version( $css_dir . $data['file'] );
			...
			if ( $data['global'] || ! $preloading_styles_enabled && is_callable( $data['preload_callback'] ) && call_user_func( $data['preload_callback'] ) ) {
				if ( filter_var( get_theme_mod( $this->schema( 'settings-inline_css-slug' ), $this->schema( 'settings-inline_css-default' ) ), FILTER_VALIDATE_BOOLEAN ) === false ) {
	 ...
	 */
	public function filter_resource_hints( array $urls, $relation_type ) {
		if ( 'preconnect' === $relation_type && wp_style_is( 'groundwp-fonts', 'queue' ) ) {
			$urls[] = [
		...
		$handles   = array_filter(
			...
			function ( $handle ) use ( $css_files ) {
				...
				if ( ! $is_valid ) {
					...
					_doing_it_wrong( __CLASS__ . '::print_styles()', esc_html( sprintf( __( 'Invalid theme stylesheet handle: %s', 'groundwp' ), $handle ) ), 'GroundWP 2.0.0' );
				}
	 ...
	 * {@see 'groundwp_preloading_styles_enabled'} filter can be used to tweak the return value.
	 *
	...
	protected function preloading_styles_enabled() {
		$preloading_styles_enabled = ! groundwp()->is_amp();
		 ...
		 */
		return apply_filters( 'groundwp_preloading_styles_enabled', $preloading_styles_enabled );
	}
	...
	protected function get_css_files() {
		 ...
		 */
		$css_files = apply_filters( 'groundwp_css_files', $css_files );
	...
	protected function get_google_fonts() {
		 ...
		 */
		$this->google_fonts = (array) apply_filters( 'groundwp_google_fonts', $google_fonts );
