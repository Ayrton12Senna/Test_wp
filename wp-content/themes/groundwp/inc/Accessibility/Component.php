<?php
/**
 * GroundWP\GroundWP\Accessibility\Component class
 *
 * @package groundwp
 */

namespace GroundWP\GroundWP\Accessibility;
...
class Component implements Component_Interface {
	...
	public function action_enqueue_navigation_script() {
		...
		wp_enqueue_script(
			...
			[],
			groundwp()->get_asset_version( get_theme_file_path( '/assets/js/navigation.min.js' ) ),
			false
		);
		wp_script_add_data( 'groundwp-navigation', 'async', true );
		wp_script_add_data( 'groundwp-navigation', 'precache', true );
		wp_localize_script(
			'groundwp-navigation',
			'groundwpScreenReaderText',
			[
				'expand'   => __( 'Expand child menu', 'groundwp' ),
				'collapse' => __( 'Collapse child menu', 'groundwp' ),
			]
		);
	}
