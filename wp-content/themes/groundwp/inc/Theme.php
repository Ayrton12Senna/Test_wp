...
class Theme {
			...
			if ( ! $component instanceof Component_Interface ) {
				throw new InvalidArgumentException(
					sprintf(
						...
						__( 'The theme component %1$s does not implement the %2$s interface.', 'groundwp' ),
						gettype( $component ),
	 ...
	 *
	 * Calling `groundwp()` is a short-hand for calling this method on the main theme instance. The instance then allows
	...
	public function component( $slug ) {
		if ( ! isset( $this->components[ $slug ] ) ) {
			throw new InvalidArgumentException(
				sprintf(
					/* translators: %s: slug */
					__( 'No theme component with the slug %s exists.', 'groundwp' ),
					$slug
				)
			);
		}
	...
	protected function get_default_components() {
		$components = [
			new Prefix_Manager\Component( '_', [ 'default' => 'groundwp' ] ),
			new Localization\Component(),
			new Base_Support\Component(),
			new Scripts\Component(),
			new Svg_Inline\Component(),
			...
			new Container_Layout\Component(),
			new Blog_Layout\Component(),
			new Meta_Box\Component(),
			new Header\Component(),
			new Call_To_Action\Component(),
			new ScrollTop\Component(),
			new Related_Posts\Component(),
			new Selection_Popup\Component(),
			new Infinite_Scroll\Component(),
		];
